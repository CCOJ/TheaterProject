...
public class Controller implements Application_API
{
	...
	private Controller()
	{
		cL_Gui = CL_Gui.getInstanceOf();
		theater = Theater.getInstanceOf();
		running = true;
	}
	...
	{
		cL_Gui.printClientsList(theater.getClientList());
	}
	...
	{
		cL_Gui.printCustomerList(theater.getCustomerList(), theater.getCreditCardList());
	}
	...
	{
		cL_Gui.printAllShowsList(theater.getShowsList());
	}
	...
	public void printAllTicketsForGivenDay()
	{
		...
		cL_Gui.printTicketListByDate(theater.getTicketList(), date);
	}
	...
	public void sellAdvanceTickets()
	{
		...
		if(userInput != null)
		{
			...
			Customer customer = theater.getCustomerList().getCustomer(customerID);
			Show show = theater.getShowsList().getShow(date);
			CreditCard creditCard = theater.getCreditCardList().getCreditCard(cardNumber);
			ShowList showsList = theater.getShowsList();
			...
			if(customer != null)
			{
				if(creditCard != null)
				{
					if(show != null)
					{
						if(showsList.isTicketPurchasedInAdvance(show))
						{
		...
		if(userInput != null)
		{
			...
			if(customer != null)
			{
				if(creditCard != null)
				{
					if(show != null)
					{
						if(!showsList.isTicketPurchasedInAdvance(show)) //If not purchase in advance, then regular ticket
						{
							Client client = theater.getClientList().getClient(show.getClientID());
